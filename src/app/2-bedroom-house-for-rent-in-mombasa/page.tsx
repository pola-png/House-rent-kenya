...
async function getProperties() {
  const { data } = await supabase.from('properties').select('*').or('location.ilike.%mombasa%,city.ilike.%mombasa%').eq('bedrooms', 2).in('status', ['Available', 'For Rent']).order('createdAt', { ascending: false }).limit(12);
  if (!data) return [];
...
export default async function Page() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-lg text-muted-foreground mb-8">Discover {properties.length}+ 2 bedroom properties for rent in Mombasa. Coastal living at its best.</p>
      {properties.length > 0 ? (<><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">{properties.map((property) => <PropertyCard key={property.id} property={property} />)}</div><div className="text-center"><Button asChild size="lg"><Link href="/search?q=mombasa&beds=2">View All Mombasa Properties</Link></Button></div></>) : (<p className="text-center py-12">No properties available. <Link href="/search" className="text-primary underline">Browse all properties</Link></p>)}
    </div>
  );
}
