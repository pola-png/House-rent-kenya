...
async function getProperties() {
  const { data } = await supabase.from('properties').select('*').eq('bedrooms', 1).in('status', ['Available', 'For Rent']).order('createdAt', { ascending: false }).limit(12);
  if (!data) return [];
...
export default async function Page() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-lg text-muted-foreground mb-8">Discover {properties.length}+ 1 bedroom apartments for rent in Kenya. Affordable options for singles & young professionals.</p>
      {properties.length > 0 ? (
        <>
