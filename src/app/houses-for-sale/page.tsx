...
async function getProperties() {
  const { data } = await supabase.from('properties').select('*').in('property_type', ['House', 'Townhouse', 'Villa', 'Mansion']).in('status', ['For Sale']).order('createdAt', { ascending: false }).limit(12);
  if (!data) return [];
...
export default async function Page() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-4">Houses for Sale in Kenya - Buy Your Dream Home</h1>
      <p className="text-lg text-muted-foreground mb-8">Discover {properties.length}+ houses for sale in Kenya. Standalone houses, townhouses, villas & luxury homes available for purchase.</p>
      {properties.length > 0 ? (
        <>
          ...
          <div className="text-center">
            <Button asChild size="lg">
              <Link href="/search?property_type=house&type=sale">View All Houses for Sale</Link>
            </Button>
          </div>
        </>
      ) : (
