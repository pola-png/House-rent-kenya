...
export default function BedsitterKasaraniPage() {
  ...
  const fetchProperties = async () => {
    const { data } = await supabase
      .from('properties')
      .select('*')
      .or('location.ilike.%kasarani%,city.ilike.%kasarani%')
      .lte('bedrooms', 1)
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-lg text-muted-foreground mb-8">
        Find affordable bedsitter apartments for rent in Kasarani, Nairobi. Browse {properties.length}+ verified listings with photos, prices, and agent contacts. Kasarani offers convenient access to Thika Road, shopping centers, and public transport.
      </p>
      ...
      {isLoading ? (
        ...
        <p className="text-center py-12">No bedsitters available in Kasarani at the moment. <Link href="/search" className="text-primary underline">Browse all properties</Link></p>
      )}
    </div>
  );
}
