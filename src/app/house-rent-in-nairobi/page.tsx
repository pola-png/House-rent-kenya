...
async function getProperties() {
  const { data } = await supabase.from('properties').select('*').or('location.ilike.%nairobi%,city.ilike.%nairobi%').eq('status', 'For Rent').order('createdAt', { ascending: false }).limit(12);
  if (!data) return [];
