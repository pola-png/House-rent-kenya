...
async function getProperties() {
  const { data } = await supabase.from('properties').select('*').eq('bedrooms', 3).in('status', ['Available', 'For Rent']).order('createdAt', { ascending: false }).limit(12);
  if (!data) return [];
...
export default async function Page() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-4">3 Bedroom Rent in Kenya - Family Homes</h1>
      <p className="text-lg text-muted-foreground mb-8">Browse {properties.length}+ 3 bedroom properties for rent in Kenya. Spacious homes for families.</p>
      {properties.length > 0 ? (<><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">{properties.map((property) => <PropertyCard key={property.id} property={property} />)}</div><div className="text-center"><Button asChild size="lg"><Link href="/search?beds=3&type=rent">View All 3 Bedroom Properties</Link></Button></div></>) : (<p className="text-center py-12">No properties available. <Link href="/search" className="text-primary underline">Browse all properties</Link></p>)}
    </div>
  );
}
