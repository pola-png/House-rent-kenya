...
async function getProperties() {
  const { data } = await supabase.from('properties').select('*').or('location.ilike.%meru%,city.ilike.%meru%').eq('bedrooms', 3).eq('status', 'For Rent').order('createdAt', { ascending: false }).limit(12);
  if (!data) return [];
