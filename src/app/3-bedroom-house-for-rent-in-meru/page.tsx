...
async function getProperties() {
  const { data } = await supabase.from('properties').select('*').or('location.ilike.%meru%,city.ilike.%meru%').eq('bedrooms', 3).in('status', ['Available', 'For Rent']).order('createdAt', { ascending: false }).limit(12);
  if (!data) return [];
...
export default async function Page() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-4">3 Bedroom House for Rent in Meru</h1>
      <p className="text-lg text-muted-foreground mb-8">Find {properties.length}+ spacious 3 bedroom houses for rent in Meru. Family-friendly homes with modern amenities.</p>
      {properties.length > 0 ? (
        <>
          ...
          <div className="text-center">
            <Button asChild size="lg">
              <Link href="/search?q=meru&beds=3">View All Meru Properties</Link>
            </Button>
          </div>
        </>
      ) : (
